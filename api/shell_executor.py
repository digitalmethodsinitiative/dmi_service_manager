...
if config_data.get('4CAT_DATASETS_PATH', False):
    ...
    shell2http.register_command(endpoint="whisper_local", command_name=f"docker run --rm -v {fourcat_path}:/app/data/ {'--gpus all ' if config_data.get('WHISPER_GPU', False) else ''}whisper whisper")
else:
...
if config_data.get('UPLOAD_FOLDER_PATH', False):
    ...
    shell2http.register_command(endpoint="whisper_remote", command_name=f"docker run --rm -v {uploads_path}:/app/data/ {'--gpus all ' if config_data.get('WHISPER_GPU', False) else ''}whisper whisper")
else:
