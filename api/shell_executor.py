...
if config_data.get('4CAT_DATASETS_PATH', False):
    ...
    shell2http.register_command(endpoint="whisper_local", command_name=f"docker run -v {fourcat_path}:/app/data/ --gpus all -it whisper whisper")
else:
...
if config_data.get('UPLOAD_FOLDER_PATH', False):
    ...
    shell2http.register_command(endpoint="whisper_remote", command_name=f"docker run -v {uploads_path}:/app/data/ --gpus all -it whisper whisper")
else:
